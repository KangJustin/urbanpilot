...
across climate resilience, accessibility, and housing, and see 2026/2040/2075 scenarios —
2026 backed by a real Street View/satellite photo of the site, 2040/2075 generated by
Midjourney.
...
server/                   Node/Express backend
  ...
  services/
    ...
    midjourneyMcpClient.js OAuth + connection management for Midjourney's MCP server
...
**Data flow for an analysis:** `LocationSearch` resolves an address to `{placeId, formattedAddress,
latitude, longitude, viewport}` (the single source of truth, `selectedLocation` in `App.js`) →
`/api/analyze` runs the climate/accessibility/housing/urbanDesign/vision agents in parallel,
each just asking Claude about whatever site it's given → results populate the Plan/Risks/Future/Ask AI
tabs. The 2026 scenario card shows a real photo (Street View if covered, else satellite) fetched
through `/api/location/street-view-image` and `/api/location/satellite-image` — these proxy
routes exist so the Google API key never reaches the browser. 2040/2075 generate via Midjourney,
using that same real photo as a composition reference by default (or your own uploaded photo).
...
browser window opens asking you to log in and authorize (use your Mega-plan account). After
that, tokens are cached in `server/.mcp-auth/` (gitignored) and refresh automatically — you
won't need to log in again unless that cache is deleted.
...
- **Mock Berkeley demo data** (`src/mock/berkeleyAnalysis.json`) is still used in two places: as
  the placeholder content shown before you search any address, and as a fallback if a live
  analysis fails *specifically for Berkeley*. Any other address that fails shows a real error
  instead of substituted mock data.
